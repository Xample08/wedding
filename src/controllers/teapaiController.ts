...
export async function adminCreateTeapai(body: unknown) {
    ...
    const type =
        typeRaw === "PUBLIC" || typeRaw === "FAMILY" ? typeRaw : "FAMILY";
    const invitation_side =
        invitationSideRaw === "GROOM" || invitationSideRaw === "BRIDE"
            ? invitationSideRaw
            : "GROOM";

    if (numberOfGuests === null || numberOfGuests <= 0) {
        throw Object.assign(new Error("number_of_guests must be > 0"), {
            status: 400,
        });
    }
...
export async function guestUpdateTeapai(
    token: string,
    body: unknown,
    ip?: string,
    ua?: string,
    ...
    const { display_name, expected_attendance, is_attending, teapai } = body;
    ...
    if (isAttendingNum === 1 && expectedNum <= 0) {
        throw Object.assign(
            new Error("Expected attendance must be at least 1"),
            { status: 400 },
        );
    }
    ...
    if (
        ...
        throw Object.assign(new Error("Session must be pagi or malam"), {
            status: 400,
        });
    }
...
export async function adminUpdateTeapai(token: string, body: unknown) {
    if (!isRecord(body)) {
        throw Object.assign(new Error("Invalid JSON body"), { status: 400 });
    }
    ...
    const updated = await teapaiService.updateTeapaiInvitation(token, {
        name,
        numberOfGuests,
        type,
        invitation_side,
    ...
    if (!updated) {
        throw Object.assign(
            new Error(
                "Invitation not found, already responded, or already deleted",
            ),
            { status: 404 },
        );
    }
...
export async function adminDeleteTeapai(token: string) {
    const deleted = await teapaiService.deleteTeapaiInvitation(token);
    if (!deleted) {
        throw Object.assign(
            new Error("Invitation not found or already deleted"),
            { status: 404 },
        );
    }
    return { success: true };
...
export async function adminImportTeapai(rows: any[]) {
    ...
    for (const row of rows) {
        try {
            ...
            const maxGuestsRaw =
                row.max_guests || row["Max Guests"] || row.number_of_guests;
            ...
            const type =
                typeRaw === "PUBLIC" || typeRaw === "FAMILY"
                    ? typeRaw
                    : "FAMILY";
            const invitation_side =
                sideRaw === "GROOM" || sideRaw === "BRIDE" ? sideRaw : "GROOM";
            ...
            await teapaiService.createTeapaiInvitation({
                ...
                type,
                invitation_side,
                adminNote: adminNoteRaw
                    ? sanitizeText(String(adminNoteRaw), 255)
                    : undefined,
                createdBy: "import",
            });
            successCount++;
